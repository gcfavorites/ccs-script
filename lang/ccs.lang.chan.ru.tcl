...
if {[pkg_info lang [list $_name $_lang] on]} {
	...
	set_text -type args -- $_lang chanset {[<+|->флаг] | [<параметр> <значение>] ...}
	set_text -type help -- $_lang chanset {Изменение параметров канала}
	set_text -type help2 -- $_lang chanset {
		{Изменение параметров канала (например, «+bitch» или «flood-chan 5:10»). Аналог команды «chanset» в консоли. Возможно назначение нескольких параметров и флагов в одной строке.}
		{Значения, содержащие пробелы, должны быть заключены в кавычки. Кавычки, находящиеся в тексте, в начале или конце слова, должны быть экранированы знаком «\»}
	}
	...
	set_text $_lang $_name #134 "Для параметра \002%s\002 нету значения"
	set_text $_lang $_name #135 "Флаги: \002%s\002 не определены"
	set_text $_lang $_name #136 "Параметры: \002%s\002 не определены"
	set_text $_lang $_name #137 "\002%s\002 (\"\002%s\002\" -> \"\002%s\002\")"
	set_text $_lang $_name #138 "Для канала \002%s\002 изменены значения флагов : %s"
	set_text $_lang $_name #139 "Для канала \002%s\002 изменены значения параметров : %s"
