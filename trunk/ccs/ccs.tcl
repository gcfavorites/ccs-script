...
namespace eval ::ccs {
	...
	# set ccs(args,lang,command)  - Аргументы использования команды для указанного языка (строка);
	# set ccs(help,lang,command)  - Помощь по команде для указанного языка (строка);
	# set ccs(help2,lang,command) - Расширенная помощь по команде для указанного языка (список строк);
	# set ccs(group,command)      - Принадлежность команды к определенной группе (строка);
	# set ccs(use,command)        - Включение/отключение команды (1 - включена, 0 - отключена),
	#                               значение по умолчанию 1 (если параметр не указан);
	# set ccs(use_auth,command)   - Команда требует авторизацию для её использования (1 - авторизация требуется,
	#                               0 - нет), значение по умолчанию 1 (если параметр не указан);
	# set ccs(use_chan,command)   - Команда использует имя канала, для каких либо действий, в случае управления
	#                               ботом через приват данная команда потребует ввести имя канала над которым 
	#                               производиться действие. Значение по умолчанию 1 (если параметр не указан)
	#                               0 - канал не используеться;
	#                               1 - указание канала обязательно;
	#                               2 - указание канала либо "*" обязательно;
	#                               3 - указание канала не обязательно;
	# set ccs(flags,command)      - Флаги доступа к команде. Если флаги указываются через вертикальную черту,
	#                               это будет означать доступность команды, как для локальных, так и для
	#                               глобальных пользователей. Если же флаг указывается одной буквой, то
	#                               использовать команду смогут только пользователи с глобальным флагом.
	#                               (дополнительно добавлен один флаг: %v - при этом пользователь должен быть
	#                               добавлен в юзерлист);
	# set ccs(alias,command)      - Список команд на которые бот будет реагировать для использования данной
	#                               команды "%pref_" будет заменена на префикс по умолчанию.
	# set ccs(block,command)      - Необязательный параметр блокировка использования команды по времени.
	#                               Задается время в секундах, через которое будет доступно повторное выполнение
	#                               команды.
	# set ccs(regexp,command)     - Регулярное выражение, выбирающее данные и передающее обрабатываемой процедуре
	#############################################################################################################
