...
if {$ccs(lang,name,$modname,$modlang)} {
	...
	set ccs(help,en,update) {Checks for new available script/module version.}
	set ccs(help2,en,update) {
		{Checks for new available script/module version.}
		{\002list\002 - view a list of new releases. \002-cur\002 updates will be shown only to installed modules and languages; \002-mod\002 - output only modules; \002-lang\002 - output only languages}
		{\002update\002 - Update all previously installed modules and languages} {\002download\002 - download the modules and languages. This should indicate a pattern: \002type\002 - type file (mod, lang, scr); \002name\002 - loadable module name; \002lang\002 - language (only with the type indicated lang. A template can be used *)}
	}
	
	set ccs(args,en,help) {[flags]}
	set ccs(help,en,help) {Help info for CCS-commands}
	set ccs(help2,en,help) {
		{Help info for CCS-commands.}
		{Flags: \002-a\002 minimal/required access; \002-l\002 lists all available commands (limited by access level); \002-с\002 aliases for commands; \002-g group\002 help for all commands in the given group}
		{Valid group are: \002%groups\002. Example: \002-p -l -g mode\002}
	}
	
	
	#############################################################################################################
	# ‘разы используемые в скрипте
	...
	set ccs(text,ccs,en,#122) "\002%s\002 is found in the bot's userlist, but don't have any known host. Please check out the hosts-list for \002%s\002."
	set ccs(text,ccs,en,#123) "Can't find \002%s\002 in the bot's userlist."
	set ccs(text,ccs,en,#124) "\002%s\002 founded in bot's userlist but matching to \002%s\002. Please check out the hosts-list for \002%s\002 and \002%s\002."
	set ccs(text,ccs,en,#125) "Can't find \002%s\002 on IRC and in the userlist."
	set ccs(text,ccs,en,#126) "You has been auto logged out because left all monitored channels."
	...
	set ccs(text,ccs,en,#200) "You should specify the channel to get help for commands of your access level"
